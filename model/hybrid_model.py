...
def load_hybrid_checkpoint(path: str | Path, map_location):
    """Load a tensor-only checkpoint from a trusted local path."""
    return torch.load(path, map_location=map_location, weights_only=True)


class HybridUniSELightning(pl.LightningModule):
    """Paper-inspired hybrid discriminative/generative/fusion study.

    The module explores the paper's high-level data flow. It is not an official
    implementation or a paper-exact reproduction. Unpublished or unavailable
    components are surfaced as explicit implementation choices.
    """
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.lm_objective_config = dict(lm_objective)
    ...
    def enhance(
        ...
        if checkpoint is not None:
            ...
            self.load_state_dict(_checkpoint_state_dict(checkpoint_data), strict=True)
        if wav.ndim == 1:
            wav = wav.unsqueeze(0)
        output = self.forward(
            wav.to(self.device),
