...
class LLM_SFT(CustomLlamaModel):
    ...
    def forward(
        ...
        global_end = global_ids.size(1)
        semantic_start = global_end + 1
        semantic_end = semantic_start + semantic_ids.size(1)
        global_logits = logits[:, :global_end, :]
        global_targets = target_ids[:, :global_end]
