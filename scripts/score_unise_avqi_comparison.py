...
def write_samples(speakers, condition_dirs, conditions, samples_dir, max_pairs):
    samples_dir = Path(samples_dir)
    complete = {pair_id: tasks for pair_id, tasks in speakers.items() if "cs" in tasks and "sv" in tasks}
    if len(complete) < 3:
        raise ValueError("At least three complete CS/SV pairs are required for a listening set")
    selected = sorted(complete.items())[: int(max_pairs)]
    for pair_id, tasks in selected:
    ...
    manifest = {
        "selection": "first pair IDs in lexicographic order",
        "max_pairs": int(max_pairs),
        "selected_pair_ids": [pair_id for pair_id, _ in selected],
        "conditions": list(conditions),
    }
    samples_dir.mkdir(parents=True, exist_ok=True)
    (samples_dir / "manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
    print(f"[SAVE] {len(selected)} listening pairs under {samples_dir}")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=5,
        metavar="{3,4,5}",
        help="Number of deterministic paired listening examples to export (default: 5).",
    )
