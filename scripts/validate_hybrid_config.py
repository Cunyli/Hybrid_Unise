    ...
    *,
    ...
    for split in ("train_kwargs", "val_kwargs", "test_kwargs"):
        ...
        if dataset_type == "hybrid_unise_webdataset_stream":
            for required_key in ("split_root", "simulation_config"):
                require(
                    required_key in kwargs,
                    f"dataset_config.{split}.{required_key} is required for hybrid_unise_webdataset_stream",
                    errors,
                )
        if dataset_type == "hybrid_unise_webdataset_fixed_recipe":
            for required_key in ("recipe_manifest", "simulation_config"):
                require(
                    required_key in kwargs,
                    f"dataset_config.{split}.{required_key} is required for hybrid_unise_webdataset_fixed_recipe",
                    errors,
                )
